  ...
  return (
    <footer className="relative flex flex-col items-center justify-center overflow-hidden bg-black/70 px-5 py-12 sm:px-10 lg:px-20">
      ...
      <div className="flex w-full max-w-[1440px] flex-col items-center gap-10 lg:flex-row lg:justify-center lg:gap-8">
        ...
        <p className="max-w-sm text-center font-heading text-3xl font-extrabold leading-tight text-foreground sm:text-4xl lg:w-96 lg:text-left lg:text-5xl">
          {footer.tagline}
        </p>
        <div className="hidden h-24 w-px bg-white/50 lg:block lg:h-36" aria-hidden />
        <nav className="flex w-full max-w-xl flex-wrap items-center justify-center gap-6 lg:w-auto lg:justify-start lg:gap-8">
          {footer.links.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="min-w-24 font-body text-lg font-medium text-foreground hover:text-gold sm:text-2xl"
            >
              {link.label}
            </Link>
          ))}
